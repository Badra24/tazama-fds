...
import { normalizeTransaction } from '../helpers/normalizeTransaction';
...
export const execute = async (reqObj: unknown): Promise<void> => {
  ...
  try {
    ...
    // This ensures legacy rules can access both FIToFIPmtSts and FIToFICstmrCdtTrf
    const normalizedRequest = {
      ...request,
      transaction: normalizeTransaction(request.transaction),
    };

    ruleRes = await handleTransaction(normalizedRequest, determineOutcome, ruleRes, loggerService, ruleConfig, databaseManager);
